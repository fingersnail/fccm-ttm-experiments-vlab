...
#define AOCX_FILE "linebuffer.aocx"
...
int main() {
    ...
    int i;
    int num_elem_A = (I + KY - 1) * (J + KX - 1);
    int num_elem_B = KY * KX;
    int num_elem_C = I * J;

    srand(time(0));
    for (int i = 0; i < I + KY - 1; i++) {
        for (int j = 0; j < J + KX - 1; j++) {
            input[i][j] = rand() % 1000;
        }
    }
    for (int i = 0; i < KY; i++) {
        for (int j = 0; j < KX; j++) {
            weight[i][j] = (1.0 * rand()) / RAND_MAX;
        }
    }
    for (int i = 0; i < I; i++) {
        for (int j = 0; j < J; j++) {
     ...
     for (int j = 0; j < J; j++) {
       ...
       for (int ky = 0; ky < KY; ky++) {
            for (int kx = 0; kx < KX; kx++) {
                c += input[i + ky][j + kx] * weight[ky][kx];
            }
       }
       if (c != result[i][j]) {
         passed = 0;
         printf("\n[FAILED]: i:%d j:%d result[i][j]:%f, right answer: %f\n", i, j, result[i][j], c);
       }
     }
   }
