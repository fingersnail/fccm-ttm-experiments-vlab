...
#define define_flat_linebuffer(\
  ...
  STEP_0, STEP_1, STEP_2, STEP_3, ELE_SIZE) \
    \
    ...
    \
                    for (size_t s = 0; s < STENCIL_STRIDE; s++) { \
                        ...
                        for (size_t e = 0; e < ELE_SIZE; e++) { \
                            write_channel_intel(output_channel, linebuffer[e][offset]); \
                        } \
                    } \
                    read_length = STEP_0; \
...
#define define_unrolled_flat_linebuffer_fast(\
  type, input_channel, output_channel, IMG_EXTENT_0, IMG_EXTENT_1, IMG_EXTENT_2, IMG_EXTENT_3,\
  STENCIL_EXTENT_0, STENCIL_EXTENT_1, STENCIL_EXTENT_2, STENCIL_EXTENT_3, ELE_SIZE) \
      \
    /*Calculate input extent*/ \
    const size_t INPUT_EXTENT_0 = STENCIL_EXTENT_0 + IMG_EXTENT_0 - 1; \
    const size_t INPUT_EXTENT_1 = STENCIL_EXTENT_1 + IMG_EXTENT_1 - 1; \
    const size_t INPUT_EXTENT_2 = STENCIL_EXTENT_2 + IMG_EXTENT_2 - 1; \
    const size_t INPUT_EXTENT_3 = STENCIL_EXTENT_3 + IMG_EXTENT_3 - 1; \
    \
    /* Calculate Stride */\
    const size_t STRIDE_0 = 1; \
    const size_t STRIDE_1 = STRIDE_0 * INPUT_EXTENT_0; \
    ...
    /* size_t stencil_start = 0, stencil_end = 0; */ \
    /* The length calculation is wrong */ \
    const size_t LINEBUFFER_EXTENT = (IMG_EXTENT_0 - 1) * STRIDE_0 + (IMG_EXTENT_1 - 1) * STRIDE_1 \
                                   + (IMG_EXTENT_2 - 1) * STRIDE_2 + (IMG_EXTENT_3 - 1) * STRIDE_3 + 1; \
    /* allocate the linebuffer */ \
    type linebuffer[ELE_SIZE][LINEBUFFER_EXTENT]; \
    \
    /* The initialization of linebuffer */ \
                    ...
                    for (size_t e = 0; e < ELE_SIZE; e++) { \
                        for (size_t dim_3 = 0; dim_3 < (unsigned) IMG_EXTENT_3; dim_3++) { \
                            for (size_t dim_2 = 0; dim_2 < (unsigned) IMG_EXTENT_2; dim_2++) { \
                                for (size_t dim_1 = 0; dim_1 < (unsigned) IMG_EXTENT_1; dim_1++) { \
                                    for (size_t dim_0 = 0; dim_0 < (unsigned) IMG_EXTENT_0; dim_0++) { \
                                        size_t offset = dim_3 * STRIDE_3 + dim_2 * STRIDE_2 \
                                                    + dim_1 * STRIDE_1 + dim_0 * STRIDE_0; \
                                        offset = (start_address + offset) % LINEBUFFER_EXTENT; \
                                        write_channel_intel(output_channel[dim_1][dim_0], linebuffer[e][offset]); \
                                    } \
                                } \
